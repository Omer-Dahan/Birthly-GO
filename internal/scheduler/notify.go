...
// app/scheduler/notify.py's send(). One deliberate divergence: if the event
...
func SendReminder(ctx context.Context, bot *gotgbot.Bot, db *sql.DB, user *models.User, event *models.Event, rule *models.ReminderRule, logID int64, occurrence time.Time, trackCalendarType string, broadcastRatePerSec int) bool {
	...
	for attempt := 0; attempt < 3; attempt++ {
		limiter.Acquire()
		var err error
		if event.PhotoFileID != nil && *event.PhotoFileID != "" {
			_, err = bot.SendPhoto(user.ID, gotgbot.InputFileByID(*event.PhotoFileID), &gotgbot.SendPhotoOpts{
				Caption:             text,
				ReplyMarkup:         kb,
				ParseMode:           "HTML",
